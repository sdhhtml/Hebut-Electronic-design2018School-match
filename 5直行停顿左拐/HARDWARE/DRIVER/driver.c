...
void Driver_Init1(void)
{
	...
	GPIO_InitStructure.GPIO_Mode = GPIO_Mode_Out_PP; 		//推挽输出
	GPIO_InitStructure.GPIO_Speed = GPIO_Speed_50MHz;		//IO口速度为50MHz
	GPIO_Init(GPIOF, &GPIO_InitStructure);					//根据设定参数初始化GPIOC
	GPIO_SetBits(GPIOF,GPIO_Pin_8);						 	//PC0输出高 顺时针方向  DRIVER_DIR
	GPIO_ResetBits(GPIOF,GPIO_Pin_9);						//PC2输出低 使能输出  DRIVER_OE
}
...
{		 					 
  ...
  //设置该引脚为复用输出功能,输出TIM8 CH2的PWM脉冲波形
	GPIO_InitStructure.GPIO_Pin = GPIO_Pin_8; //TIM8_CH2
	GPIO_InitStructure.GPIO_Mode = GPIO_Mode_AF_PP;  //复用推挽输出
	GPIO_InitStructure.GPIO_Speed = GPIO_Speed_50MHz;
	...
	TIM_ITConfig(TIM1, TIM_IT_Update ,ENABLE);  //TIM8   使能或者失能指定的TIM中断
