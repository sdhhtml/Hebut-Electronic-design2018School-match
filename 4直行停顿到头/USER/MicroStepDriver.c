...
static void TIM_Mode_Config(void)
{
	...
	TIM_OCInitTypeDef  TIM_OCInitStructure0;
  // 开启定时器时钟,即内部时钟CK_INT=72M
	MSD_PULSE_TIM_APBxClock_FUN(MSD_PULSE_TIM_CLK,ENABLE);

    /*--------------------时基结构体初始化-------------------------*/

	TIM_TimeBaseStructInit(&TIM_TimeBaseStructure0);
    // 自动重装载寄存器的值，累计TIM_Period+1个周期后产生一个更新或者中断
	TIM_TimeBaseStructure0.TIM_Period=MSD_PULSE_TIM_PERIOD;	
	// 驱动CNT计数器的时钟 = Fck_int/(psc+1)
	TIM_TimeBaseStructure0.TIM_Prescaler= MSD_PULSE_TIM_PSC;	
	// 时钟分频因子 ，配置死区时间时需要用到
	TIM_TimeBaseStructure0.TIM_ClockDivision=TIM_CKD_DIV1;		
	// 计数器计数模式，设置为向上计数
	TIM_TimeBaseStructure0.TIM_CounterMode=TIM_CounterMode_Up;		
	// 重复计数器的值，最大值为255
	//TIM_TimeBaseStructure.TIM_RepetitionCounter=0;	
	// 初始化定时器
	TIM_TimeBaseInit(MSD_PULSE_TIM, &TIM_TimeBaseStructure0);

	/*--------------------输出比较结构体初始化-------------------*/		

	// 配置为PWM模式2
	TIM_OCInitStructure0.TIM_OCMode = TIM_OCMode_PWM2;
	// 输出使能
	TIM_OCInitStructure0.TIM_OutputState = TIM_OutputState_Enable;
	// 互补输出禁能
	TIM_OCInitStructure0.TIM_OutputNState = TIM_OutputNState_Disable; 
	// 设置占空比大小
	TIM_OCInitStructure0.TIM_Pulse = MSD_PULSE_TIM_PERIOD/2;
	// 输出通道电平极性配置
	TIM_OCInitStructure0.TIM_OCPolarity = TIM_OCPolarity_Low;
	// 输出通道空闲电平极性配置
	TIM_OCInitStructure0.TIM_OCIdleState = TIM_OCIdleState_Reset;
    
	MSD_PULSE_OCx_Init(MSD_PULSE_TIM, &TIM_OCInitStructure0);
    //使能TIM1_CH1预装载寄存器
...
void MSD_Move(signed int step, unsigned int accel, unsigned int decel, unsigned int speed)
{
    ...
    if(step == 1)
    {
        ...
        //设置占空比为50%	
        TIM_SetCompare4(MSD_PULSE_TIM,Pulse_width>>1);
        //使能定时器	      
...
if (TIM_GetITStatus(MSD_PULSE_TIM, TIM_IT_Update) != RESET)
{
  ...
  srd.step_delay = new_step_delay;
	...
	if(stepPosition%100==0)printf("步进电机0当前位置:%d\r\n",stepPosition);
	if(stepPosition>6279)stepPosition=0;
	if(stepPosition<(-6279))stepPosition=0;
}
